import { and, desc, eq, gt, inArray, or, type SQL } from "drizzle-orm";
import { nanoid } from "nanoid";
import type { Message, Task, TaskStatus } from "@sparstrow/shared";
import { getDb } from "../db/connection.js";
import { agents, messages, runs, tasks } from "../db/schema.js";
...
export function createTask(input: TaskCreateInput): Task {
  ...
  db.insert(tasks)
    .values({
      ...
      status: input.assignedAgentId ? "todo" : "inbox",
...
export function listTasks(filter: { status?: string; projectId?: string; assignedAgentId?: string }): Task[] {
