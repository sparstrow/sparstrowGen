...
- [ ] T014 [US2] Create `packages/core/src/api/routes/host-fs.ts` with `GET /host-fs/volumes` and `GET /host-fs/dirs`, including the FR-022b loopback refusal on each handler, and the status codes in [contracts/host-fs-api.md](./contracts/host-fs-api.md). **Give the loopback refusal and an OS permission denial distinguishable error bodies** — both are `403`, but one is a trust-boundary refusal and the other is an ordinary locked folder, and a client that cannot tell them apart will show security wording for a `System Volume Information` click
