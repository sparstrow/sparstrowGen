...
| :------------- | :--------------- | :------- | :----------------------------- |
| `path`         | string           | Yes      | The path to the customization directory (for `entries`) or another JSON config file (for `inherits`). |
| `include_only` | array of strings | No       | A list of regex patterns. If specified, only customizations whose directory names match at least one of these patterns will be loaded. |
| `exclude`      | array of strings | No       | A list of regex patterns. Customizations whose directory names match any of these patterns will be skipped. |
