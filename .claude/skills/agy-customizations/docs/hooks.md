...
| :--------------- | :---------------- | :--------------- | :----------------- |
| `PreToolUse`     | Before a tool step executes. | Tool name (e.g., `run_command`). | Grouped (uses `matcher` & `hooks` wrapper). |
| `PostToolUse`    | After a tool step completes. | Tool name (e.g., `run_command`). | Grouped (uses `matcher` & `hooks` wrapper). |
| `PreInvocation`  | Before the model is called. | N/A (ignored). | Flat (list of handler objects directly). |
| `PostInvocation` | After tool calls finish. | N/A (ignored). | Flat (list of handler objects directly). |
| `Stop`           | When the execution loop terminates. | N/A (ignored). | Flat (list of handler objects directly). |
