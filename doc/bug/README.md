...
| [`BUG-2026-08-24-project-provision-always-400s`](BUG-2026-08-24-project-provision-always-400s.md) | 🔴 open | Every "New project" creation path 400s unconditionally — `POST /api/v1/projects/provision` spreads two client-only fields (`mode`, `gitInit`) straight into the `projects` insert; neither is a real column. A workspace cannot create its first project at all, with no error shown in the UI. Found live during `T-VR-06`'s verification pass; unrelated to the Vite retirement, deliberately not fixed there |
