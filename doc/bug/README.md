...
| [`BUG-2026-08-22-chat-new-session-404s`](BUG-2026-08-22-chat-new-session-404s.md) | 🔴 open | `/chat`'s "Send message" on a new conversation 404s — `POST /api/v1/chat/sessions` has no route at all (real or stub), so the composer shows a bare "The model failed / Not Found". Found live on staging during T-M11-02; worked around with `POST /runs` directly |
...
| [`BUG-2026-08-22-desktop-servicemanager-health-check-times-out`](BUG-2026-08-22-desktop-servicemanager-health-check-times-out.md) | 🔴 open | The Electron shell's `ServiceManager` reports its supervised local core as unhealthy in all three T-M11-04 launches, even though the core's own log shows it genuinely became ready well inside the 60s deadline. Cosmetic today (`main.ts` opens the window regardless), root cause not isolated |
...
| [`BUG-2026-08-22-core-tests-flake-under-turbo-parallelism`](BUG-2026-08-22-core-tests-flake-under-turbo-parallelism.md) | 🔴 open | `pnpm test --force` at the repo root intermittently times out up to 25 `@sparstrow/core` tests across ten unrelated files (graph client/lifecycle/tools, viz-manager, run-manager finalize, git-status, host-fs) — likely the same five-way turbo contention as the realtime-events flake, but manifesting inside `@sparstrow/core` itself. Not investigated |
