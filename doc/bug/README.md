...
| [`BUG-2026-08-25-project-variants-read-queries-a-table-that-does-not-exist`](BUG-2026-08-25-project-variants-read-queries-a-table-that-does-not-exist.md) | 🔴 open | `GET /projects/:id/variants` and `useProjectVariants` query `project_variants`, a table that does not exist anywhere in `packages/shared/src/db/schema.ts` or its migrations — a variant is really a `projects` row with `parentProjectId` set. Found while converting `useCreateVariant` in `T-WA-02`; the read is out of scope for that task (writes only), so left as found. The new `createVariantAction` inserts into `projects` correctly, so this only affects listing existing variants |
| [`BUG-2026-08-25-skills-list-file-count-is-nan`](BUG-2026-08-25-skills-list-file-count-is-nan.md) | 🔴 open | The Skills list page shows "NaN" in the Files column for every skill — `GET /skills` never returns a `fileCount`/`file_count` field (no such column on `skills`; only the detail route joins `skill_files`), so `skill.fileCount + 1` is `undefined + 1`. Found live while verifying `T-WA-05`; pre-existing, unrelated to that task's write-only conversion |
| [`BUG-2026-08-25-skill-detail-page-always-crashes`](BUG-2026-08-25-skill-detail-page-always-crashes.md) | 🔴 open | Every skill's detail page (`/skills/<id>`) crashes on load with "Cannot read properties of undefined (reading 'map')" — `GET /skills/:id` never joins `skill_files`, so `skill.files` is `undefined` and `skill-detail.tsx`'s file-tree builder throws unconditionally. Unchanged since M3 (#80); found live while verifying `T-WA-05`, which could not exercise its detail-page actions through the UI as a result |
