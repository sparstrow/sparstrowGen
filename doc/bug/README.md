...
| [`BUG-2026-08-23-agent-creator-duplicate-user-bubble`](BUG-2026-08-23-agent-creator-duplicate-user-bubble.md) | 🔴 open | A fresh Agent Creator interview briefly shows the owner's first message twice (an optimistic bubble plus an early `messages` refetch that already contains the persisted row) before settling once the reply arrives — intake 0008's exact race, on the one page that never got `chat-pending.ts`'s fix. Found live while verifying M13; not caused by M13, not fixed there (page's own logic is out of scope) |
