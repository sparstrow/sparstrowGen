...
| [`BUG-2026-08-20-flaky-realtime-live-events-test`](BUG-2026-08-20-flaky-realtime-live-events-test.md) | 🔴 open | `pnpm test` fails intermittently on one `apps/web` test — a cold dynamic `import()` inside the first test body times out under turbo's five-way parallel run. Turbo-only; never reproduces standalone |
