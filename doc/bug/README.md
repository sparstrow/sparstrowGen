...
| [`BUG-2026-08-23-headless-spawn-skill-leak`](BUG-2026-08-23-headless-spawn-skill-leak.md) | 🟡 investigating | Every real chat turn on a real, credentialed, paired machine failed — not `G-31`'s assumed "no credentials". A headless CLI spawn inherits the operator's full personal `~/.claude` config unisolated; a machine with a preamble-tier personal skill installed can never grant it the tool permission it wants (no TTY), so claude-code stalls to its 120s timeout and antigravity denies immediately. Fixed with `--disable-slash-commands` on every headless spawn (both providers); live confirmation pending |
