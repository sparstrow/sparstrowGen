...
| [`BUG-2026-08-18-orphaned-account-rows-on-staging`](BUG-2026-08-18-orphaned-account-rows-on-staging.md) | 🔴 open | Staging holds 8 orphaned account trees — profile rows, workspaces and memberships for auth users that no longer exist, unreachable by any RLS policy. `auth.admin.deleteUser` does not cascade |
