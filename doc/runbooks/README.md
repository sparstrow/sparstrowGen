...
| ✅ done | ~~Raise **"Database connection pool size"**~~ — **ruled out, not the cause** | **Raised 2→10 live on 2026-08-28 with your go-ahead; ruled out.** Waited for the resulting client-disconnect cycle to finish, then re-ran both reproductions fresh: the real paired daemon's `terminal.list` still times out through the actual app, and the Realtime Inspector's `authenticated`-role self-broadcast test still never returns — identical symptom to before the change. A `postgres`-role control on the same topic still works instantly, confirming the service itself is healthy and this isn't a resource-starvation issue on this dimension. This was the strongest untried lead; with it closed, the remaining path is a Supabase support ticket — see the bug file. | [`../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md) |
| 🔲 pending | File a **Supabase support ticket** for `authenticated`-role Realtime broadcast delivery failing on `pnymngoqseltgigcfevq` | **Added 2026-08-28, `T-DI-05`, after both dashboard-setting hypotheses (public-access toggle, connection pool size) were ruled out live.** Everything queryable or adjustable from an agent session has been checked — RLS correctness, client races, topic/event naming, client library version, `self` config, schema grants, and both Realtime Settings toggles above. What's left needs Supabase's own infrastructure team: as `postgres`, presence and self-broadcast both round-trip in under a second; as `authenticated`, impersonating a real user RLS already grants access to, the join succeeds but the same connection never receives even its own broadcast. This reproduction is minimal, doesn't depend on this repo's code, and is written up ready to paste into a ticket. Full diagnosis: [`doc/bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md). Until this clears, `T-DI-05` cannot complete, and — if the same pattern affects `010`/`015`'s identical `private: true` usage — run transcripts and chat turn deltas may be silently non-live in production today too. | [`../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md) |
