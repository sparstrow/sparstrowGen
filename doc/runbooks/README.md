...
|---|---|---|---|
| 🔲 pending | Apply **`018` (re-run), `019` and `020`** from `packages/shared/drizzle/policies/` | **Added 2026-08-27 (`T-DI-02`).** These create the daemon's Supabase Auth identity table, the `private.current_daemon_scope()` helper, the four `realtime.messages` policies that are the machine's half of the terminal channels, and the `bootstrap_workspace()` guard that stops a daemon identity minting a junk workspace. **An agent cannot apply them**: the Supabase CLI is not logged in on this machine (`supabase projects list` → "Access token not provided") and the `supabase` MCP server needs an interactive OAuth grant. Either run `supabase login` and apply the three files in order, or authorize the MCP server (`/mcp`) so an agent can. Each file ends with a `-- Verify` block written to be pasted as-is — `019`'s expects **ten** rows in `pg_policies` for `schemaname = 'realtime'` and names all ten. **`020` replaces `bootstrap_workspace()` wholesale** (Postgres cannot patch a body in place); it is `004`'s function plus one guard, verified by diff — re-diff before applying if `004` has changed. Until this lands, `T-DI-03`'s live checks and all of `T-DI-05` are blocked. | [`../tasks/DI/T-DI-02-daemon-identity-schema.md`](../tasks/DI/T-DI-02-daemon-identity-schema.md) |
| ⛔ blocked | ~~Re-export **`SUPABASE_JWT_SIGNING_KEY`**~~ — superseded 2026-08-27, this action is not possible | **Added 2026-08-26 (T-M16-02); superseded 2026-08-27, confirmed live with the owner in the Supabase dashboard.** This row used to ask you to re-export the current ES256 signing key's private half as JSON from **Project Settings → API → JWT Keys**. Walked that screen together: the "Key Details" modal only ever shows the *public* key set (`key_ops: ["verify"]`, no `d`), and a freshly created standby ES256 key showed the identical public-only shape at the moment of creation — no export, no one-time reveal, anywhere. This is not a dashboard-permissions issue; Supabase's asymmetric JWT Signing Keys are designed so the private half never leaves their infrastructure. **There is nothing to paste in here** — `mintRealtimeToken()`'s whole approach (read the project's own private key from an env var, sign with it ourselves) needs redesigning, not a corrected value. Tracing this also found a second, independent blocker in the RLS itself: see `doc/KnownGaps.md` `G-48` and [`doc/bug/BUG-2026-08-27-daemon-realtime-token-cannot-pass-terminal-channel-rls.md`](../bug/BUG-2026-08-27-daemon-realtime-token-cannot-pass-terminal-channel-rls.md). **Nothing for you to do on this row right now** — a redesign is being scoped; this row will change again once there's a concrete owner action (if any) attached to it. | — |
