...
|---|---|---|---|
| 🔲 pending | Re-export **`SUPABASE_JWT_SIGNING_KEY`** — a recent rotation broke it on **both** Preview and Development | **Added 2026-08-26 (T-M16-02); updated 2026-08-27 during `T-M17-02`'s and `T-M17-06`'s live passes.** This project signs with an ES256 key pair, not the legacy shared HS256 secret — confirmed against its own `/auth/v1/.well-known/jwks.json`, which returned exactly one `ES256` key (`kid 1ee0a572-eaf2-4110-b43f-9f60462fdec7`). `/api/daemon/realtime/token` needs that key's **private** half, as JSON, to mint the short-lived credential a paired machine uses to hold M16's control channel. **`vercel env ls preview` shows this is ONE value tagged `Preview, Development` together, not two independent secrets** — narrowing this to "Development only" on 2026-08-27 (`T-M17-02`) was wrong, corrected same day by `T-M17-06`: a real paired daemon against the **band 21** preview deployment hit the identical 500/no-`kid` failure `T-M17-02` found locally. Cross-referenced against `G-47`'s 2026-08-26 evidence (401, not 500, on the band-20 preview) and this value's own `vercel env ls` "updated 13h ago" timestamp, the value was **replaced with a malformed one sometime early on 2026-08-27** — a regression, not a gap that was always there. Re-export the **current** ES256 signing key as JSON from **Project Settings → API → JWT Keys** and set it on both Preview and Development — only a project admin can do this. **Production is unconfirmed either way.** Full detail: `doc/KnownGaps.md` `G-48`. | — |
