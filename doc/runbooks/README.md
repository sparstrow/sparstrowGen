...
| ⏳ awaiting reply | ~~File a **Supabase support ticket**~~ — **filed 2026-08-28, watch for their reply** | **Submitted from your own Supabase account** — `sparstrowgen` org, `sparstrowgen-staging` project, "APIs and client libraries" / Realtime / JavaScript, severity Normal. Confirmed sent; replies go to `domains@sparstrow.com` (no ticket number surfaced in the dashboard at submission — the reply email is the reference for now). Nothing left for either of us to do here until Supabase responds — everything queryable or adjustable from this end (RLS, client races, topic/event naming, client version, `self` config, schema grants, both Realtime Settings toggles) has been checked and was included in the ticket. Full diagnosis and the exact ticket text: [`doc/bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md). Until this clears, `T-DI-05` cannot complete, and — if the same pattern affects `010`/`015`'s identical `private: true` usage — run transcripts and chat turn deltas may be silently non-live in production today too. | [`../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md) |
