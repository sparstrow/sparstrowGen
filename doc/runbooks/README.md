...
| 🔲 pending | Check **Project Settings → Realtime → "Allow public access"** at [the project's Realtime Settings](https://supabase.com/dashboard/project/pnymngoqseltgigcfevq/realtime/settings) and disable it if enabled | **Added 2026-08-28, `T-DI-05`.** With `018`/`019`/`020` applied and two independent, real client-side bugs also fixed the same session, a real paired daemon and a real signed-in browser both reach `SUBSCRIBED` on the terminal control channel with correct RLS authorization — and a broadcast sent on it still never arrives at the subscriber. Reproduced with two synthetic connections using only pre-existing, already-shipped M16 policies, so this is not specific to the daemon or to this band. Supabase's own docs state plainly that private channels need this dashboard-only toggle disabled to be enforced; it cannot be read or changed via SQL, the Management API paths tried, or the `supabase` CLI — this session found no way to check or flip it itself. **Not a confirmed fix, the strongest untried lead**: full investigation, everything ruled out, and what to do if this doesn't resolve it are in [`doc/bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md). Until this clears, `T-DI-05` cannot complete, and — if the same pattern affects `010`/`015`'s identical `private: true` usage — run transcripts and chat turn deltas may be silently non-live in production today too. | [`../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md`](../bug/BUG-2026-08-28-private-broadcast-channels-not-relaying.md) |
