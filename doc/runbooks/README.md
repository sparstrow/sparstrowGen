...
|---|---|---|---|
| 🔲 pending | Set **`SUPABASE_JWT_SIGNING_KEY`** on the Vercel project (all three environments) | **Added 2026-08-26 (T-M16-02).** This project signs with an ES256 key pair, not the legacy shared HS256 secret — confirmed against its own `/auth/v1/.well-known/jwks.json`, which returned exactly one `ES256` key (`kid 1ee0a572-eaf2-4110-b43f-9f60462fdec7`). `/api/daemon/realtime/token` needs that key's **private** half, as JSON, to mint the short-lived credential a paired machine uses to hold M16's control channel — get it from **Project Settings → API → JWT Keys → the current signing key → export/download as JSON**, which only a project admin can do; nothing available to an agent can read a private signing key, only verify against its already-public JWKS. Until this is set, `mintRealtimeToken()` throws by name and the route returns a 500 — nothing else in M16 is blocked on it. | — |
