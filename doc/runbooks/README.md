...
|---|---|---|---|
| 🔲 pending | Fix **`SUPABASE_JWT_SIGNING_KEY`** on the Vercel project's **Development** environment — Preview is fine | **Added 2026-08-26 (T-M16-02); narrowed 2026-08-27 during `T-M17-02`'s live pass.** This project signs with an ES256 key pair, not the legacy shared HS256 secret — confirmed against its own `/auth/v1/.well-known/jwks.json`, which returned exactly one `ES256` key (`kid 1ee0a572-eaf2-4110-b43f-9f60462fdec7`). `/api/daemon/realtime/token` needs that key's **private** half, as JSON, to mint the short-lived credential a paired machine uses to hold M16's control channel. **Preview is set correctly** — `G-47` already found it minting past the auth check on the band-20 preview (401, not 500). **Development is set but malformed**: `vercel env pull --environment=development` for this project returns a JSON value whose `kid` field is empty or absent, so `mintRealtimeToken()` throws its own named "no `kid`" error rather than the "not set" one — confirmed live, 2026-08-27, pulling the real value into a scratch `.env.local` and running a real paired daemon against it locally (removed after; nothing committed). This is what blocks a live shell-typing pass on `localhost`, distinct from the Preview-environment gap `G-47` already tracks. Re-export the **current** ES256 signing key as JSON from **Project Settings → API → JWT Keys** and set it again specifically on the Development environment — only a project admin can do this. **Production is unconfirmed either way.** | — |
