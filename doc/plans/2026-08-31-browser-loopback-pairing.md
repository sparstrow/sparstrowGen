...
| US1 | Machines page: `PairingCodePanel` (the code + countdown + generate-a-code button) removed outright — nothing on this page can start a pairing attempt anymore, only the CLI can. Empty state and header become instructional (the `sparstrow pair` command, no code). `PairingOutcomePanel`'s "paired" variant is kept as-is, reacting to a new runtime id the same way it always did; its "expired" variant is dropped along with the code it was about | What the owner sees on the Machines page — informational only, since a pairing attempt now has no representation the page can observe until the machine actually appears |
