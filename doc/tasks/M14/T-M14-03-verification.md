...
- [ ] **US2 scenario 1** — Given no machine has ever been paired in this
...
- [ ] **US2 scenario 2a (offline)** — Given a paired machine is currently
      ...
      is not lost
- [ ] **US2 scenario 2b (comes back online)** — the SAME turn from 2a
      resolves automatically once the machine reconnects, within the TTL —
      no resend, no reload required for it to start resolving (a reload MAY
      be what surfaces the result, per FR-007, but the turn must not need a
      fresh send)
- [ ] **US2 scenario 3** — Given a Project-context session where no paired
...
- [ ] **TTL expiry** — a turn forced past `wait_expires_at` (via the SQL
...
- [ ] The browser console has no errors across all three scenarios
...
- [ ] Each of the three waiting cards is visually distinct from the others
      (not just distinct text in an identical box) — scenario 1 vs 2a vs 3
      must not require reading carefully to tell apart
- [ ] `TurnExpiredNotice` is visually distinct from `TurnErrorBanner`
...
- [ ] A turn that fails with a genuine provider error (not a TTL expiry)
...
- [ ] SC-001/SC-004's already-live-proven pieces from T-M13-05 (Realtime
      ...
      re-broken — spot-check via the existing regression tests, not a full
      re-walk

## C — What can be verified today

- [ ] All three waiting-reason scenarios, and the TTL-expiry case, per
...
- [ ] Tick 18.14 in [`../MasterTaskQueue.md`](../MasterTaskQueue.md) and mark
      Band 18's M14 rows (18.12–18.14) done
- [ ] Update [`M14/README.md`](README.md)'s **Status** row and task table
- [ ] Update the plan's own **Status** row
      ([`../../plans/2026-08-23-chat-message-sending.md`](../../plans/2026-08-23-chat-message-sending.md))
- [ ] Knowledge Center pass per `AGENTS.md` §3.2 — already done in T-M14-02;
      confirm nothing this task found changes it further
...
<!-- Filled in when the task lands: what was actually run, what it found. -->
