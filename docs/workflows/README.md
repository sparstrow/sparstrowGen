...
(Listener, Curator, Pipeline Suggester, Memory Archivist, Investigator, …); where existing
tools are good, we *extract the method* and re-author on our own infrastructure — we don't run
them (they carry their own memory/preamble/telemetry).
...
```
 ...
 you ─▶ Listener (capture) ─▶ intake ─▶ Curator (lock) ─┤
...
- **One gate** — every capture, every mode, passes through the [Curator](./agents/curator.md)
...
| Curator (analysis + routing gate, all modes) | [`agents/curator.md`](./agents/curator.md) |
...
|---|---|---|---|
| [Feedback](./feedback.md) | `feedback` | 🔒 locked 2026-07-11 | Curator → Investigate → fix |
| _New feature_ | `new-feature` | ⬜ to design | Curator → routed/gap → plan → build |
| _New concept_ | `new-concept` | ⬜ to design | Curator → routed/gap → north-star/plan |
| _New design (HTML)_ | `design` | ⬜ to design | Curator → decode → SPEC → build |
| _Feature change_ | `feature-change` | ⬜ to design | Curator → routed/gap → build |
| _Decision / pitfall / lesson / meeting / architecture_ | memory-track | ⬜ to design | Curator → Memory Archivist → vault |
...
dream-plans, agent scope-cuts, **and** Curator gap outcomes you decide to park. Record:
...
> a slash-command skill per member (e.g. `/listener`, `/curator`).
