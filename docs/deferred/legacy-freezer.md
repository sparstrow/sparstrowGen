...
### Curator pitfall-attribution (run/task history read access)
- source: agent-defer
- project: factory
- what: the [Curator](docs/workflows/agents/curator.md) agent, for `pitfall`-mode captures,
  is meant to attempt attributing the pitfall to a causing agent/run — but this needs read
  access to `runs`/`tasks` history, which isn't wired into the Curator's spec yet.
- why deferred: cut during design of the Review & Routing workflow (2026-07-12) — the rest of
  the Curator/Pipeline Suggester/Memory Archivist design didn't depend on it, and it only
...
- revisit when: the Chat/Sessions feature (or any live Sparstrowgen instance) makes run/task
  history queryable from a design/build session; fold the capability into the Curator's tool
  list at that point.
- size: S   ·   date: 2026-07-12   ·   links: docs/workflows/agents/curator.md
