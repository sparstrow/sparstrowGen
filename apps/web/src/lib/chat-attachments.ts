...
/**
 * What is left here after restructure Phase 1: the **browser-side** read.
 *
 * `attachmentsByMessageId` moved to `@sparstrow/shared` because `server/`'s
 * chat route needs it. `sessionAttachments` below did not move, and the reason
 * matters: it is called with the caller's own browser Supabase client (see
 * `chat.tsx`), which makes it a client talking straight to the database — the
 * one thing AGENTS.md §1 rule 1 forbids. It keeps working exactly as before,
 * but it is debt, not a pattern: Phase 5 turns it into a `server/` route and
 * this file goes away. See `G-63` in `doc/KnownGaps.md`.
 */
